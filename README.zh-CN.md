...
- **AGENTS.md** — 像 pi、opencode、Codex、DeepSeek Harness 一样自动读取常驻指令：`~/.agents/AGENTS.md`，以及从所在 git 仓库根目录到每个工作区根目录路径上的每个 `AGENTS.md`。内容放进服务端说明；由于有的客户端不会把说明交给 AI，每个连接的第一次工具结果前面也会再附一份。子文件夹里的 `AGENTS.md` 在某次 `read_files` 或 `apply_patch` 第一次碰到该文件夹下的文件时，随结果发送一次。每块上限 32 KB（超出时先舍弃范围更大的文件）。连接期间修改、删除或新建 AGENTS.md，会随下一次工具结果告诉 AI。
- **计划 / 执行模式** — 面板标题栏状态标记旁的二选一切换。执行模式是每次启动 Bridge 时的默认模式，AI 可以修改文件、运行命令；计划模式只读，AI 可以读代码、运行白名单内的只读命令以及项目的测试和构建（如 `npm test`、`npm run build`），`apply_patch`、`send_command_input`、`terminate_command` 仍在工具列表里，但调用会返回 `READ_ONLY_MODE`。切换立即生效，AI 会在下一次工具结果里收到通知。
- **防止原地打转** — 同一个工具用完全相同的参数连续调用到第 3、5、8 次时，结果后面会附一句提醒，让 AI 换个办法或者告诉你卡在哪里（参考 DeepSeek Harness）；调用本身照常执行。
