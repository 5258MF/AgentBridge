...
**同一个 MCP 地址能接多个网页大模型吗？会不会冲突？**
可以。协议层完全隔离：每个客户端（ChatGPT / Claude / Grok 等）连接后都是独立的 MCP 会话，互不影响（容量上限 64 个会话）。但要注意它们**共享同一个工作区和工具**：
- 两个模型同时 `apply_patch` 修改同一个文件 → 后写的覆盖先写的（没有文件锁）
- `run_command` 终端会话是共享的，两个模型同时跑命令、互相输入会串台
- 建议：一个模型负责一个任务（不同文件/不同终端），或错开时间使用；面板会话区可单独「断开」某个会话